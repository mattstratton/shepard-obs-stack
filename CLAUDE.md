...
| `shepherd_session_tokens_total`        | source, git_repo, model, type        | Claude stop hook |
| `shepherd_skill_invocations_total`     | source, git_repo, skill_name, skill_type | Claude stop hook |
| `shepherd_skill_tokens_total`          | source, git_repo, skill_name, skill_type, model, type | Claude stop hook |
| `shepherd_subagent_invocations_total`  | source, git_repo, subagent_type      | Claude stop hook |
| `shepherd_mcp_calls_total`             | source, git_repo, mcp_server, mcp_tool | Claude stop hook |

`context_chars` type values: `tool_output`, `user_prompt`, `compact_summary`. Emitted from session parser output at session end.

`session_tokens`/`skill_invocations`/`skill_tokens`/`subagent_invocations`/`mcp_calls` (fork-only, spec 02) come from the parser's metrics sidecar (spec 01) via a per-session state file (`${SHEPARD_STATE_DIR:-~/.local/state/shepard-obs}/claude/<session_id>.json`) — the Stop hook fires every turn but the sidecar is cumulative, so stop.sh emits only the delta since the previous firing. `compaction_events`/`context_chars`/`context_compaction_pre_tokens` route through the same delta path (fixes an upstream bug where they were re-summed from the full session on every turn).
...
| 10 Projects & Skills       | `shepherd-projects`         | Prometheus                          | `configs/grafana/dashboards/15-projects.json`         |
...
Projects & Skills (10, fork-only) is Claude Code-only: per-repo computed cost and token attribution, top skills/slash-commands with turn-window cost attribution, subagent launches, and MCP calls — all filterable by the `$git_repo` variable. Cost panel (04) also gained a `$git_repo` variable, a "Cost Analysis" row (Native Estimate sanity check, Cache Savings, Unpriced Models guardrail, Model Mix Over Time), and its primary cost numbers now come from `shepherd:claude:computed_cost_usd` (spec 03) instead of Claude's native cost estimate.
